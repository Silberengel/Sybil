...
#[AsCommand(
    name: 'longform-feed',
    description: 'Display longform content (kind 30023) with their replies',
)]
class LongformFeedCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->logger->info('Fetching longform feed', [
            'relay' => $relay,
            'limit' => $limit,
            'author' => $author,
        ]);

        // Query for longform content and their replies
        $filters = [
            'kinds' => [30023, 1111], // Longform content and replies
            'limit' => $limit,
        ];
        ...
        try {
            ...
            $this->logger->error('Failed to fetch longform feed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
            $io->error('Failed to fetch longform feed: ' . $e->getMessage());
